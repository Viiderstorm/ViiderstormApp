...
* Integer bounces - max number of ray bounces
...
function RayTracer(view, scene, bounces, supersample){
    ...
    */
    ...
    /*
    * Recursively bounces the rays and calculates color
    * Ray ray - inital ray object to trace
    * Integer depth - the max depth of the ray
    */
    self.illuminate    = function(ray, depth){
        ...
        if(intersection == null){
            ...
            var ambientLight = self.scene.getAmbientLight(intersection, 0.4);
            if(!arrayIsNull(shadowIntersection)){
                color = ambientLight;
            } else {
                //ray intersection not in any shadow
                var diffuseLight  = self.scene.getDiffuseLight(intersection, 0.5);
                var specularLight = self.scene.getSpecularLight(intersection, ray, 0.2); 
            ...
            if( depth > 0){
                if(intersection.geometry.material.reflection > 0.0){
                    ...
                    color = color.add(self.illuminate(reflectRay, depth - 1).multiplyScalar(intersection.geometry.material.reflection));
                }
            }
        }
        return color;
...
sphereMat1.reflection = 0.5;
var sphereMat2 = new BasicMaterial(0x1D1D1D);
sphereMat2.reflection = 0.95;
...
$(document).ready(function(){
    ...
    $("#renderButtonUniform").click(function(){
        ...
        var checkSphere1 = $("#sphereOneToggleInput").prop('checked');
        var checkSphere2 = $("#sphereTwoToggleInput").prop('checked');
        var checkBounce  = parseInt($("#bounceInput").val()) || 5;
        var spp  = parseInt($("#samplePerPixelInput").val()) || 1;
        
        if(checkSphere1){
            sphereMat1.reflection = 0.95;
        } else {
            sphereMat1.reflection = 0.0;
        }
        if(checkSphere2){
            sphereMat2.reflection = 0.95;
        } else {
            sphereMat2.reflection = 0.0;
        }
        ...
        var rt = new RayTracer(view, sc, checkBounce);
        if(spp > 1){
